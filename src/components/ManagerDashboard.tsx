...
// --- SUB-COMPONENTE: SERVIÇOS (Responsivo) ---
        ...
        {fields.map((item, k) => (
          ...
          <div key={item.id} className="flex flex-col sm:flex-row gap-2 sm:items-center bg-white sm:bg-transparent p-3 sm:p-0 rounded-lg border sm:border-0 border-indigo-100 shadow-sm sm:shadow-none">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
                <input {...register(`professionals.${nestIndex}.services.${k}.price`)} type="number" placeholder="R$" className="w-1/2 sm:w-24 p-2 text-sm border rounded-lg" />
                <input {...register(`professionals.${nestIndex}.services.${k}.duration`)} placeholder="Min" className="w-1/2 sm:w-24 p-2 text-sm border rounded-lg" />
                <button type="button" onClick={() => remove(k)} className="text-red-400 p-2 bg-red-50 sm:bg-transparent rounded-lg flex items-center justify-center border border-red-100 sm:border-0">
                  <Trash2 size={16}/>
                </button>
            </div>
          </div>
...
// --- SUB-COMPONENTE: PROFISSIONAL (Responsivo) ---
const ProfessionalItem = ({ index, control, register, remove, setValue }: any) => {
  ...
  return (
    <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 shadow-sm mb-4 relative">
        <button onClick={() => remove(index)} className="absolute top-2 right-2 text-gray-300 hover:text-red-500 p-2 z-10">
            <Trash2 size={18}/>
        </button>
                  ...
                  {settingsTab === 'hours' && (
                    <div className="space-y-3">
                        {['seg', 'ter', 'qua', 'qui', 'sex', 'sab', 'dom'].map(day => { 
                            ...
                            return (
                                // LAYOUT RESPONSIVO: Flex Column no Mobile
